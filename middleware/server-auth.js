...
// ═══════════════════════════════════════════════════════════
// MIDDLEWARE — bloqueia clientes de acessar rotas operacionais
// Usar APÓS requireAuth nas rotas internas do K11
// ═══════════════════════════════════════════════════════════
...
function requireOperacional(req, res, next) {
    if (req.user?.role === 'cliente') {
        return res.status(403).json({
            ok:    false,
            error: 'Acesso restrito à equipe operacional K11.',
            code:  'ROLE_FORBIDDEN',
        });
    }
    next();
...
// ═══════════════════════════════════════════════════════════
// MIDDLEWARE — garante que só clientes acessem rotas do portal
// ═══════════════════════════════════════════════════════════
...
function requireCliente(req, res, next) {
    if (req.user?.role !== 'cliente') {
        return res.status(403).json({
            ok:    false,
            error: 'Rota exclusiva para clientes.',
            code:  'ROLE_FORBIDDEN',
        });
    }
    next();
